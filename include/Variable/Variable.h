...
public:
  ...
  /// @return the number of points hold by the variable. The number of points is
  /// updated each time the data series changes
  unsigned int nbPoints() const noexcept;

  /// Returns the real range of the variable, i.e. the min and max x-axis values
  /// of the data series between the range of the variable. The real range is
  /// updated each time the variable range or the data series changed
  ...
  void updateData(const std::vector<IDataSeries*>& dataSeries,
                  ...
                  const DateTimeRange& newCacheRange, bool notify = true);
  ...
  static QByteArray
  mimeData(const std::vector<std::shared_ptr<Variable>>& variables)
  {
...
SCIQLOP_REGISTER_META_TYPE(VARIABLE_PTR_VECTOR_REGISTRY,
                           QVector<std::shared_ptr<Variable>>)
