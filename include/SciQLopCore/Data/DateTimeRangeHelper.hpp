...
namespace DateTimeRangeHelper
{
  ...
  inline bool isPureShift(const DateTimeRange& range1,
                          const DateTimeRange& range2)
  {
    return cpp_utils::numeric::almost_equal<double>(range1.delta(),
                                                    range2.delta(), 1) &&
           !cpp_utils::numeric::almost_equal(range1.m_TStart, range2.m_TStart,
                                             1);
  }

  inline bool isPureZoom(const DateTimeRange& range1,
                         const DateTimeRange& range2)
  {
    return !cpp_utils::numeric::almost_equal<double>(range1.delta(),
                                                     range2.delta(), 1) &&
           cpp_utils::numeric::almost_equal<double>(range1.center(),
                                                    range2.center(), 1);
  }
   ...
   * InvalidDateTimeRangeTransformation if the transformation has NaN or
   * forbiden values
   */
  inline std::optional<DateTimeRangeTransformation>
  computeTransformation(const DateTimeRange& range1,
                        const DateTimeRange& range2)
  {
  ...
  inline TransformationType getTransformationType(const DateTimeRange& range1,
                                                  const DateTimeRange& range2)
  {
    ...
    if(transformation.has_value())
    {
      ...
      {
        if(transformation->shift > 0.) return TransformationType::PanRight;
        return TransformationType::PanLeft;
      }
      if(transformation->zoom > 0.) return TransformationType::ZoomOut;
